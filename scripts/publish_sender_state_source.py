...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Publish sender state source CSV to Kafka.")
    parser.add_argument("--bootstrap-servers", default="localhost:9092")
    parser.add_argument(
        "--source-dir",
        default=str(ROOT / "Data" / "logical_sources"),
        help=f"Thu muc chua file {SENDER_STATE_SOURCE_FILENAME}",
    )
...
def main() -> int:
    ...
    published = 0
    try:
        for payload in iter_sender_state_source_payloads(args.source_dir, limit=args.max_events):
            producer.send(SENDER_STATE_TOPIC, key=payload["source_event_id"], value=payload)
            published += 1
            if published % 100 == 0:
            ...
            if delay:
                time.sleep(delay)
        producer.flush()
        print(f"FINAL: Published {published} sender state source events to {SENDER_STATE_TOPIC}.", flush=True)
        return 0
    finally:
        producer.close()
