...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Publish transaction source CSV to Kafka.")
    parser.add_argument("--bootstrap-servers", default="localhost:9092")
    parser.add_argument(
        "--source-dir",
        default=str(ROOT / "Data" / "logical_sources"),
        help=f"Thu muc chua file {TRANSACTION_SOURCE_FILENAME}",
    )
...
def main() -> int:
    ...
    published = 0
    try:
        for payload in iter_transaction_source_payloads(args.source_dir, limit=args.max_events):
            producer.send(TRANSACTION_TOPIC, key=payload["event_id"], value=payload)
            published += 1
            if published % 100 == 0:
            ...
            if delay:
                time.sleep(delay)
        producer.flush()
        print(f"FINAL: Published {published} transaction source events to {TRANSACTION_TOPIC}.", flush=True)
        return 0
    finally:
        producer.close()
